...
class SimPaDataset:
    ...
    def get_litteral_diff(self, pred, name_map, name_flag, tres):
        pred = np.array(pred)
    ...
    def evaluate(
        ...
        for batch_idx in tqdm(range(num_batches), total=num_batches, desc="Evaluation: "):
            ...
            obs, true_disease, _, _, _, first_actions = self.env.initialize_state(indices=patientIndices)
            ...
            batch_diff_list = np.ones((obs.shape[0], self.env.diag_size, total_length + 1)) * invalid_action_idx
            while curr_turn < total_length:
                ...
                batch_action_list[:obs.shape[0], curr_turn + 1] = (sym_action * (1 - done)) + (invalid_action_idx * done)
        ...
        df = df.applymap(lambda x: 'None' if x == invalid_action_idx else self.env.symptom_data[self.env.symptom_index_2_key[x]]['name'])        
        
        name_map = {i: self.env.pathology_data[self.env.pathology_index_2_key[i]]['condition_name'] for i in range(self.env.diag_size)}
        ...
        df2 = df2.applymap(lambda x: self.get_litteral_diff(x, name_map, True, tres=0.01))
