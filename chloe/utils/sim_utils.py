...
def load_record_batch(filepath):
    """Utility function to load and pre-process a Synthea generated patient csv file.

    This function behaves as the `load_csv` function
    but instead of returning a panda DataFrame, it returns
    a pyarrow RecordBatch containing generated patients
    from Synthea simulator.
    ...
    -------
    rb: RecordBatch
        a pyarrow record batch containing the simuated patients.
    unique_symptoms: list
        a list of unique symptoms within the provided data.
    unique_pathologies: list
        a list of unique pathologies within the provided data.
    ...
    """
    outputs = load_csv(filepath)
    outputs[0] = pa.RecordBatch.from_pandas(outputs[0], nthreads=1)
    return outputs


def get_storage_keys(prefix=""):
    """Utility function defining the keys used for indexing the data in a shared memory.

    This function returns the keys for storing/retrieving the data
    in the shared memory based on the provided prefix.

    Parameters
    ----------
    prefix : str
        prefix to used for indexing the data in the
        plasma store socket. Default: ""
    ...
    if len(prefix) >= 10:
        raise ValueError(
            f"the len of prefix should be less than 10. got {len(prefix)}: {prefix}\n"
        )
    # chosen to be of length 10
    keys = [
        "data_frame",
        "symptom_id",
        "patholo_id",
    ...
    prefix = prefix.lower() + "|" * (10 - len(prefix))
    plasma_keys = [plasma.ObjectID((prefix + k).encode()) for k in keys]
    return plasma_keys


def store_file_in_shared_memory(socket, filepath, prefix="", replace_if_present=True):
    """Utility function for storing the data in a shared memory.

    This function stores the data contained in the provided
    file (generated patients from Synthea simulator) in a
    shared memory.

    Parameters
    ----------
    socket : str
        the plasma store socket to used for data sharing.
    filepath : str
        path to the csv file containing generated patients
        from Synthea simulator.
    prefix : str
    ...
    replace_if_present : bool
        indicate whether to replace the content of the plasma store
        with the new data file if the related keys are already
        present in the plasma store socket. Default: True

    Returns
    -------
    result: list
        a list of indices corresponding to objects stored in the
        shared memory.

    """
    try:
        client = plasma.connect(socket)
    except OSError:
        command = f"plasma_store -m 2000000000 -s {socket}"
        raise ValueError(
            f"Connection to socket failed for pathname {socket}\n"
            f"Could not connect to socket {socket}\n"
            f"Try executing the following command in a sparate shell: {command}\n"
            f"for allocating a socket with 2GB capacity.\n"
            f"Do not end that process before the end of th training.\n"
        )
    plasma_keys = get_storage_keys(prefix)
    all_key_present = True
    present_keys = []
    for key in plasma_keys:
        if client.contains(key):
            present_keys.append(key)
        else:
            all_key_present = False

    if all_key_present and not replace_if_present:
        # we don't have to do anything
        return plasma_keys

    # for consistency, delete all keys that were there
    if len(present_keys) > 0:
        decode_keys = [key.binary().decode() for key in present_keys]
        warnings.warn(
            f"Keys [{decode_keys}] are present in the shared mem."
            f" They will be deleted."
        )
        client.delete(present_keys)

    outputs = load_record_batch(filepath)
    for i in range(len(outputs)):
        object_id = plasma_keys[i]
        if i > 0:
            client.put(outputs[i], object_id, memcopy_threads=1)
        else:
            # save a record_batch
            record_batch = outputs[i]
...
def retrieve_file_from_shared_memory(socket, prefix=""):
    """Utility function for retrieving the data in a shared memory.

    This function retrieves data corresponding
    to Synthea generated patients from a shared memory.
    ...
    plasma_keys = get_storage_keys(prefix)
    for key in plasma_keys:
        if not client.contains(key):
            decode_key = key.binary().decode()
            raise ValueError(f"key [{decode_key}] is not present in the shared mem.")

    outputs = []
    pa.set_cpu_count(1)
    for i in range(len(plasma_keys)):
        object_id = plasma_keys[i]
        if i > 0:
            outputs.append(client.get([object_id])[0])
        else:
            # retrieve a record_batch
            data = client.get_buffers([object_id])[0]
            bufferdata = pa.BufferReader(data)
            # Convert object back into an Arrow RecordBatch
            reader = pa.RecordBatchStreamReader(bufferdata)
            record_batch = reader.read_next_batch()
            outputs.append(record_batch)
